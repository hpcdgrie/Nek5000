...
c add heat sink for t.
       u = vz(ix,iy,iz,ie) 
       radius = 0.5
      if(ifield.eq.2) then
        qvol = - u*2.0*qw/radius
      endif
...
c-----------------------------------------------------------------------
     ...
     & vphi2(lx1,ly1,lz1,lelt),t2(lx1,ly1,lz1,lelt)
      common /QW/ qw
      real qw
...
c-----------------------------------------------------------------------
     ...
     & vphi2(lx1,ly1,lz1,lelt),t2(lx1,ly1,lz1,lelt)
      ...
      real*8 tt2(3,nphi*nz*nr)
      real*8 vr_int(nr),vz_int(nr),t_int(nr)
      real*8 vz2_int(nr),vr2_int(nr),vphi2_int(nr),t2_int(nr)
      real*8 vr_avg(nr),vz_avg(nr),t_avg(nr),yr(nr),yr_plus(nr)
      real*8 vz2_avg(nr),vr2_avg(nr),vphi2_avg(nr),t2_avg(nr)
...
cc set up xyz
      ...
      call interp_v(tt2,xyz,n_interp,3)
...
cc return vr_avg,vz_avg,	  
      ...
      t_int(ir) = t_int(ir) + tt2(1,in)
      t2_int(ir) = t2_int(ir) + tt2(2,in)
      enddo
      enddo
      ...
      t_int(ir) = t_int(ir)/(dble(nz)*dble(nphi))
      t2_int(ir) = t2_int(ir)/(dble(nz)*dble(nphi))
      enddo
      ...
      write(6,*) "t_int(250):",t_int(250)
      write(6,*) "t_int(275):",t_int(275)
      write(6,*) "t_int(300):",t_int(300)
      ...
      if (atime.ne.0. .and. dtime.ne.0.) then
        ...
        t_avg(ir) = t_avg(ir)*alpha + t_int(ir)*beta
        t2_avg(ir) = t2_avg(ir)*alpha + t2_int(ir)*beta
        enddo
      endif
      timel = time
...
c dump averaged force to file at iostep
   ...
   10 format(I5,',',F14.10,',',F14.10,',',F14.10,',',
     ...
     & vphi2_avg(ir)/u_tau**2
      ...
      open(unit=58,file='temperature.dat')
      write(58,'(A)')
     & '   i    yr   yplus    t_avg    t_rms'
   ...
   11 format(I5,',',F14.10,',',F14.10,',',F14.10,',',
     ...
     & yr(ir),
     & yr(ir)/delta_v,
     & (t_avg(1)-t_avg(ir))/t_tau,
     & (t2_avg(ir)-t_avg(ir)**2)/t_tau**2
      enddo
...
c
     ...
     $   ('nelgt.ne.nelgv not yet supported in interp_v!$',nelgv)
      ...
      call opcopy(fwrk(1,1),fwrk(1,2),fwrk(1,3),t(1,1,1,1,1),t2,t2)
      endif
...
c-----------------------------------------------------------------------
      ...
      do i=1,ntot
         xm1(i,1,1,1) = xm1(i,1,1,1)*0.5 ! original diameter is 2, change diameter to 1
         ym1(i,1,1,1) = ym1(i,1,1,1)*0.5
      enddo
      param(59) = 1     ! force nek5 to recognize element deformation.

c this re2 file does no include BC for t.
c so add BC for t here.
      do iel=1,nelv
        do ifc=1,2*ndim 
